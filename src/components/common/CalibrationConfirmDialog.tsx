...
> = ({ open, details, language, onConfirm, onCancel }) => {
  // The shell lives out here, where `open` is in scope, so AnimatePresence can
  // still play the exit. An early `return null` would unmount the whole tree
  // on close and the dialog would vanish rather than leave.
  //
  // The body is remounted per pending calibration, so the draft inside always
  // starts from the position that was actually dragged. A useEffect resetting
  // state on prop change would do the same thing with an extra render and a
  // dependency list to keep correct.
  return (
    <ModalShell
              ...
              <div className="flex justify-between gap-3 items-start">
                ...
                <dd className="flex flex-col gap-1.5 items-end">
                  <label className="flex items-center gap-2">
                    ...
                    />
                    <span className="hud-text-muted admin-text-sm w-6">mm</span>
                  </label>
                  <label className="flex items-center gap-2">
