...
pub fn listen_for_beacon(timeout: Duration, ignore_self: bool) -> Option<Beacon> {
    ...
    loop {
        ...
        // Re-armed every iteration: a datagram we discard (our own beacon, or
        // noise from something else on the range LAN) must not extend the
        // overall deadline.
